...
### The Administrator Dashboard
  ...
  __Requirements__   
   ...
   2. An admin should be able to __add__, __delete__ and __change vehicles__ in the database.   
      ...
      * associate a vehicle price rate with a vehicle    
      * associated image/s with a vehicle (upload image/s)
      * disassociate image/s from a vehicle (delete an images)  
   ...
   4. An admin should be able to __generate reports/graphs__, such as: 
   ...
   5. Allow an admin to export and download any __generated reports/graphs__ made from existing data from the database onto their computer
   6. An admin should be able to __create a vehicle price rate__, with a __min__ and __max__ weekly price rate assigned to it
      * An admin can then associate the vehicle rate with any particular vehicle
   7. An admin should be able to see a __list of all active reservations__, __a list of all active hires__ and __a list of all kingdomhire vehicles__ when they log into the __Administrator Dashboard__
   8. An admin should be __able to log a reservation for a vehicle__, where when the reservation is logged for a date in the future, after some time has passed and the current real world date is equal or earlier than the start date of that reservation, 
   ...
   9. An admin should be able to __reset their password__ if they forget it, this will require __an email verification process__
   10. An admin should be able to __change their password__, they will need to know their __current password__ to do able to do this
   11. To allow an admin to login to the __Administrator Dashboard__, they will need to provide an __email address__ and __password__, which will need be already inserted into the database
   12. Allow an admin to export and download the content of the entire database into a spreadsheet friendly format
